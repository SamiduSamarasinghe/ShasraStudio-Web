...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $con->prepare("SELECT * FROM booking_wedding WHERE DATE(event_datetime) = ?");
    if (!$stmt) {
    ...
    // Insert the new wedding booking into the database
    $stmt = $con->prepare("INSERT INTO booking_wedding (full_name, event_type, location, event_datetime, crowd_quantity, photography_type, expected_cost, other_wantings, event_description) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    if (!$stmt) {
        echo 'SQL prepare error: ' . $con->error;
        exit();
    }
    ...
    header("Location: http://localhost/WEBSITE/service_page/wedding.html");
    exit();
}
?>
